...
class Processor:
    ...
    def __init__(self, device: torch.device, args):
        ...
        self.rop_matrix = self._load_or_create_projection_matrix(
            rop_file="weights/rop_1024_256.pt", 
            in_dim=1024,
            out_dim=256,
            seed=42
        )
        self.rop_matrix = self.rop_matrix.to(self.device)

    def _generate_orthonormal_matrix(self, in_dim=1024, out_dim=256, seed=42):
        """
        生成一个随机正交矩阵 [in_dim, in_dim]，并选取前 out_dim 列作为投影矩阵 [in_dim, out_dim]。
        """
        torch.manual_seed(seed)
        rand_mat = torch.randn(in_dim, in_dim)  # 随机矩阵
        Q, _ = torch.linalg.qr(rand_mat)        # QR 分解得到正交矩阵 Q:[in_dim, in_dim]
        return Q[:, :out_dim]                  # [in_dim, out_dim]
    def _load_or_create_projection_matrix(self, rop_file, in_dim=1024, out_dim=256, seed=42):
        """
        ...
        if os.path.exists(rop_file):
            ...
            print(f"File '{rop_file}' not found. Generating a new ROP matrix ...")
            rop_matrix = self._generate_orthonormal_matrix(in_dim, out_dim, seed)
            torch.save(rop_matrix, rop_file)
            print(f"ROP matrix saved to '{rop_file}'")
        return rop_matrix
    ...
    def process(self, dict_data: Dict[str, Any], mode: str = "train") -> Dict[str, np.ndarray]:
        ...
        hubert_hid = dict_data["hubert_hid"].to(self.device)        # [B,N,3072]
        B, N = tar_pose.shape[0], tar_pose.shape[1]
        j = self.joints
        in_hubert = in_hubert @ self.rop_matrix  # [B,N,256]
        hubert_hid = hubert_hid @ self.rop_matrix  # [B,N,256]
        ...
        print("hubert_hid:", hubert_hid.shape)
        # print("mode:", mode)
        print("B,N:", B,N)
        # print("joints:", j)
        # exit(0)
        ...
        out = {
            ...
            "hubert_hid":          _to_np_squeezed(hubert_hid),
...
def main():
    ...
    ap.add_argument("--cache_path", default="./datasets/beat2_cache2/", help="cache path for dataset")
    ...
    # 改为 pkl 目标路径
    ap.add_argument("--dst_pkl", default="./datasets/beat2_echomask_test.pkl" ,help="Destination pickle file")
    ...
    if os.path.isfile(args.dst_pkl):
        ...
        print(f"if you want to overwrite it, please delete it first.")
        return
