...
class CustomDataset(Dataset):
    def __init__(self, args, loader_type, augmentation=None, kwargs=None, build_cache=True):
        ...
        if self.args.addHubert:
            ...
            # self.hubert_model.cuda()
            if loader_type == "test":
                self.hubert_model.eval().cuda()
            else:
                self.hubert_model.cuda()
    ...
    def cache_generation(self, out_lmdb_dir, disable_filtering, clean_first_seconds,  clean_final_seconds, is_test=False):
        ...
        for index, file_name in self.selected_file.iterrows():
            ...
            if self.args.audio_rep is not None:
                ...
                if self.args.audio_rep == "onset+amplitude":
                    ...
                    if self.args.addHubert:
                        ...
                        hubert_feat = None
                        hubert_feat, hubert_hid = self.get_hubert_from_16k_speech_long(self.hubert_model, self.wav2vec2_processor,
                        ...
                        hubert_hid = hubert_hid.squeeze(0).numpy()
            ...
            filtered_result = self._sample_from_clip(
                dst_lmdb_env,
                audio_each_file_onset,beat_audio, hubert_feat, hubert_hid, pose_each_file, trans_each_file, shape_each_file, facial_each_file, word_each_file,
    ...
    def _sample_from_clip(
        self, dst_lmdb_env, audio_each_file,beat_audio,hubert_feat,hubert_hid, pose_each_file, trans_each_file, shape_each_file, facial_each_file, word_each_file,
        vid_each_file, emo_each_file, sem_each_file,
        disable_filtering, clean_first_seconds, clean_final_seconds, is_test,
        ):
        """
            ...
            for i in range(num_subdivision): # cut into around 2s chip, (self npose)
                ...
                if self.args.audio_rep is not None:
                    ...
                    if self.args.addHubert:
                        ...
                        sample_audio_hubert_hid = hubert_hid[audio_start_hubert:audio_end_hubert]
                else:
                ...
                if sample_pose.any() != None:
                    ...
                    if is_correct_motion or disable_filtering:
                        ...
                        if self.args.addHubert:
                            ...
                            sample_audio_hubert_hid_list.append(sample_audio_hubert_hid)
                    else:
                        n_filtered_out[filtering_message] += 1

            if len(sample_pose_list) > 0:
                with dst_lmdb_env.begin(write=True) as txn:
                    for pose, audio,beat,hubert, hubert_hid, facial, shape, word, vid, emo, sem, trans in zip(
                        sample_pose_list,
                        sample_audio_list,
                        sample_audio_beat_list,
                        sample_audio_hubert_list,
                        ...
                        v = [pose, audio, beat, hubert, hubert_hid, facial, shape, word, emo, sem, vid, trans]
    ...
    def __getitem__(self, idx):
        with self.lmdb_env.begin(write=False) as txn:
            ...
            tar_pose, in_audio, in_beat, in_hubert, in_hubert_hid, in_facial, in_shape, in_word, emo, sem, vid, trans = sample
            #print(in_shape)
            ...
            in_hubert_hid = torch.from_numpy(in_hubert_hid.copy()).float()
            ...
            return {"pose":tar_pose, "audio":in_audio,"beat":in_beat,"hubert":in_hubert, "hubert_hid":in_hubert_hid, "facial":in_facial, "beta": in_shape, "word":in_word, "id":vid, "emo":emo, "sem":sem, "trans":trans}
