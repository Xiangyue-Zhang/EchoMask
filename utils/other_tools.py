...
def load_checkpoints2(model,
    ...
    """
    通用加载函数：
    1) 自动识别 state_dict 存放位置（支持: 直接 state_dict, 或 {'state_dict':..., 'model_state':..., 'model':..., ...}）
    2) 自动处理 DataParallel 的 'module.' 前缀缺失/多余
    3) 失败时回退到 strict=False 并打印 missing/unexpected 键，便于定位问题
    """
    states = torch.load(save_path, map_location=map_location)
    ...
    if isinstance(states, dict):
        # 常见的封装键
        candidate_keys = [
            'model_state', 'model_state_dict', 'state_dict', 'model', 'net',
            'generator', 'module', 'ema', 'weights', 'params'
        ]
        for k in candidate_keys:
            if k in states and _looks_like_state_dict(states[k]):
                state_dict = states[k]
                if verbose:
                    print(
                        f"[load_checkpoints] Detected nested state_dict under key '{k}'."
                    )
                break
        # 如果没有匹配到，就看它本身是不是 state_dict
        if state_dict is None and _looks_like_state_dict(states):
            state_dict = states
            if verbose:
                print("[load_checkpoints] Using top-level dict as state_dict.")
    else:
        raise ValueError(f"Unsupported checkpoint format: type={type(states)}")

    if state_dict is None:
        raise ValueError(
            "Could not find a valid state_dict in the checkpoint.")

    # 2) 构造三种前缀方案
    def strip_module(sd):
        return OrderedDict((k[7:], v) if k.startswith('module.') else (k, v)
                           for k, v in sd.items())

    def add_module(sd):
        return OrderedDict((k if k.startswith('module.') else f'module.{k}', v)
                           for k, v in sd.items())

    trials = [
        ('as_is', state_dict),
        ('strip_module', strip_module(state_dict)),
        ('add_module', add_module(state_dict)),
    ]

    # 3) 依次尝试 strict=True
    for name, sd in trials:
        try:
            res = model.load_state_dict(sd, strict=strict)
            if strict:
                # PyTorch 在 strict=True 时成功返回 None
                if verbose:
                    print(
                        f"[load_checkpoints] Loaded ({name}) with strict=True."
                    )
                print(f"Successfully loaded checkpoints for {load_name}.")
                return True
            else:
                # strict=False 返回 IncompatibleKeys(missing_keys, unexpected_keys)
                missing = getattr(res, 'missing_keys', [])
                unexpected = getattr(res, 'unexpected_keys', [])
                if verbose:
                    print(
                        f"[load_checkpoints] Loaded ({name}) with strict=False."
                    )
                    print(
                ...
                print(
                    f"Successfully loaded checkpoints for {load_name} (strict=False)."
                )
                return True
        except RuntimeError as e:
            if verbose:
                print(
                    f"[load_checkpoints] Failed on ({name}) with strict={strict}: {e}"
                )

    # 4) 如果 strict=True 三次都失败，降级到 strict=False 再试一次三种方案
    if strict:
        if verbose:
            print("[load_checkpoints] Falling back to strict=False ...")
        for name, sd in trials:
            try:
                ...
                if verbose:
                    print(
                        f"[load_checkpoints] Failed on ({name}) with strict=False: {e}"
                    )

    # 5) 全部失败
    print(
        "Error: could not load checkpoints with any key-prefix strategy. Please check layer names and shapes."
    )
    return False
