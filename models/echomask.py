...
class MaskGenerator(nn.Module):
    ...
    def __init__(self, embed_dim=768, nhead=4, num_layers=8):
        ...
        # self.film_layers = nn.ModuleDict(
        #     {key: FiLM(embed_dim)
        #      for key in ["hands", "upper", "lower"]})
        # # 融合权重（可学习），平衡共性与独特信息
        self.fusion_weights = nn.Parameter(torch.ones(3) * 0.33)
    ...
    def forward(self,
        ...
        if need_weights:
            ...
            for i, key in enumerate(["hands", "upper", "lower"]):
                ...
                # fused_attn = weight * common_attn + (
                #     1 - weight) * unique_attn  # (B, T)
                fused_attn_list.append(common_attn)
                # fused_attn_list.append(fused_attn)
                out_attn_list.append(out_attn)
            # 融合各条件得到最终帧级重要性评分
            for i, key in enumerate(["hands", "upper", "lower"]):
                fused_attn_list[
                    i] = fused_attn_list[i] * self.fusion_weights[i]
                out_attn_list[i] = out_attn_list[i] * self.fusion_weights[i]
            fused_attn_final = torch.stack(fused_attn_list,
                                           dim=0).mean(dim=0)  # (B, T)
            mask_frame = Soft2HardMaskSTE.apply(fused_attn_final, mask_ratio,
                                                hard_ratio, soft_ratio)
            out_attn_final = torch.stack(out_attn_list,
                                         dim=0).mean(dim=0)  # (B, T, T_q)
            return out_attn_final, mask_frame
        else:
            out_attn_list = []
            for i, key in enumerate(["hands", "upper", "lower"]):
                cond = conditions[key]  # (B, T_q, embed_dim)
                # 共享 transformer decoder 得到共性 attention map
                out_attn = self.shared_transformer_decoder(
                    cond, motion, need_weights=need_weights)
                # cross_attn_shared: (num_layers, B, tgt_len, src_len), tgt_len = T_q, src_len = T
                out_attn_list.append(out_attn)
            for i, key in enumerate(["hands", "upper", "lower"]):
...
class echomask(nn.Module):

    def __init__(self, args=None):
        super(echomask, self).__init__()
        self.args = args
        ######## constractive loss ########
        ...
        self.audio_feature2motion_upper = nn.Linear(args.audio_f,
                                                    768)  # 256 to 768
        self.audio_feature2motion_hands = nn.Linear(args.audio_f,
                                                    768)  # 256 to 768
        self.audio_feature2motion_lower = nn.Linear(args.audio_f,
                                                    768)  # 256 to 768
    ...
    def gen_attention_map(self,hubert=None,
                       ...
                       use_word=True,
                       in_id=None,):
        cond_face, cond_hands, cond_upper, cond_lower = self.MAM.get_cond(
                hubert, hubert_hid)
    ...
    def gen_mask_frame(self,
        ...
        cond_face, cond_hands, cond_upper, cond_lower, loss_moau = self.MAM(
            hubert, hubert_hid, face_zq, hands_zq, upper_zq, lower_zq)
        ...
        if use_word:
            ...
            word_hints, mask_frame = self.mask_generator(
                motion=motion_refined_embeddings_in,
                conditions=cond,
    ...
    def forward(self,
        ...
        if train_mode:
            ...
            cond_face, cond_hands, cond_upper, cond_lower, loss_moau = self.MAM(
                hubert, hubert_hid, face_zq, hands_zq, upper_zq, lower_zq)
            # t_end = time.time()
            # print('MAM time:', t_end - t_start)
            
        else:
            cond_face, cond_hands, cond_upper, cond_lower = self.MAM.get_cond(
                hubert, hubert_hid)
            loss_moau = torch.zeros((), device=cond_face.device)
        ...
        if use_body_hints:
            ...
            fusion_face_a = torch.cat([cond_face, body_hint_face], dim=2)
        ...
        if use_word:
            ...
            word_hints = self.mask_generator(
                motion=motion_refined_embeddings_in, conditions=cond)
        ...
        ########## beat cons loss ###########
        beat_cons_loss = self.beat_cons_loss(hands_latent_prezq, cond_hands)
        ...
        # t_mmm_end = time.time()
        # print('mmm time:', t_mmm_end - t_mmm_start)
        # print('face_latent:', face_latent.shape)
        # exit()
        return {
...
class MAM(nn.Module):
    ...
    def __init__(self, args=None):
        ...
        self.lower_decoder_high = nn.TransformerDecoder(
            self.transformer_de_layer, num_layers=1)
        self.predict_motion_face = MLP(256, 256, 256)
        self.predict_motion_hands = MLP(256, 256, 256)
        self.predict_motion_upper = MLP(256, 256, 256)
        self.predict_motion_lower = MLP(256, 256, 256)
        self.match_face_classifier = MLP(512, 256, 2)
        self.match_hands_classifier = MLP(512, 256, 2)
        self.match_upper_classifier = MLP(512, 256, 2)
        self.match_lower_classifier = MLP(512, 256, 2)
        self.position_embeddings = PeriodicPositionalEncoding(
            256,
            period=self.args.pose_length,
            max_seq_len=self.args.pose_length)
        self.position_embeddings_hubert = PeriodicPositionalEncoding(
            256,
            period=self.args.pose_length,
            max_seq_len=self.args.pose_length)
        self.loss_fn = nn.MSELoss()
        self.match_loss_fn = nn.CrossEntropyLoss()
    ...
    def forward(self,
        ...
        in_high = self.position_embeddings_hubert(hubert)
        in_low = self.position_embeddings_hubert(hubert_hid)
        ...
        moau_query_face = self.position_embeddings_hubert(moau_query_face)
        moau_query_hands = self.position_embeddings_hubert(moau_query_hands)
        moau_query_upper = self.position_embeddings_hubert(moau_query_upper)
        moau_query_lower = self.position_embeddings_hubert(moau_query_lower)
        ...
        q_face_2_in = self.face_decoder_low(tgt=q_face_1, memory=in_low)
        q_hands_2_in = self.hands_decoder_low(tgt=q_hands_1, memory=in_low)
        q_upper_2_in = self.upper_decoder_low(tgt=q_upper_1, memory=in_low)
        q_lower_2_in = self.lower_decoder_low(tgt=q_lower_1, memory=in_low)
        ...
        q_face_3_in = self.face_decoder_high(tgt=q_face_2, memory=in_high)
        q_hands_3_in = self.hands_decoder_high(tgt=q_hands_2, memory=in_high)
        q_upper_3_in = self.upper_decoder_high(tgt=q_upper_2, memory=in_high)
        q_lower_3_in = self.lower_decoder_high(tgt=q_lower_2, memory=in_high)
        ...
        if circle_loss:
            ...
            q_face_p = self.predict_motion_face(q_face_r)
            q_hands_p = self.predict_motion_hands(q_hands_r)
            q_upper_p = self.predict_motion_upper(q_upper_r)
            q_lower_p = self.predict_motion_lower(q_lower_r)

            q_face_loss = self.loss_fn(q_face_p, out_motion_face)
            q_hands_loss = self.loss_fn(q_hands_p, out_motion_hands)
            q_upper_loss = self.loss_fn(q_upper_p, out_motion_upper)
            q_lower_loss = self.loss_fn(q_lower_p, out_motion_lower)

            match_face = torch.cat(
                [q_face_r.mean(dim=1),
                 out_motion_face.mean(dim=1)], dim=1)
            match_hands = torch.cat(
                [q_hands_r.mean(dim=1),
                 out_motion_hands.mean(dim=1)], dim=1)
            match_upper = torch.cat(
                [q_upper_r.mean(dim=1),
                 out_motion_upper.mean(dim=1)], dim=1)
            match_lower = torch.cat(
                [q_lower_r.mean(dim=1),
                 out_motion_lower.mean(dim=1)], dim=1)
            match_face_p = self.match_face_classifier(match_face)
            match_hands_p = self.match_hands_classifier(match_hands)
            match_upper_p = self.match_upper_classifier(match_upper)
            match_lower_p = self.match_lower_classifier(match_lower)
            # 这里认为每个样本是匹配的，对应标签为 1 (例如 [0,1] 中的类别1)
            target_match = torch.ones(bs,
                                      dtype=torch.long,
                                      device=hubert.device)
            match_face_loss = self.match_loss_fn(match_face_p, target_match)
            match_hands_loss = self.match_loss_fn(match_hands_p, target_match)
            match_upper_loss = self.match_loss_fn(match_upper_p, target_match)
            match_lower_loss = self.match_loss_fn(match_lower_p, target_match)
        else:
            q_face_loss = torch.tensor(0)
            q_hands_loss = torch.tensor(0)
            q_upper_loss = torch.tensor(0)
            q_lower_loss = torch.tensor(0)
        loss_match = match_face_loss + match_hands_loss + match_upper_loss + match_lower_loss
        loss_q = q_face_loss + q_hands_loss + q_upper_loss + q_lower_loss
        loss_clip = q_face_clip + q_hands_clip + q_upper_clip + q_lower_clip
        loss_all = loss_q + loss_clip + loss_match
    ...
    def get_cond(self, hubert, hubert_hid):
        in_high = hubert
        in_low = hubert_hid
        bs, t, c = in_high.shape
        moau_query_face = self.moau_query_face.repeat(bs, 64, 1)
        moau_query_hands = self.moau_query_hands.repeat(bs, 64, 1)
        moau_query_upper = self.moau_query_upper.repeat(bs, 64, 1)
        moau_query_lower = self.moau_query_lower.repeat(bs, 64, 1)
