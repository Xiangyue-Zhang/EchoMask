...
class RVQVAE_3D(nn.Module):
    ...
    def decode(self, x1d_s=None, x1d_t=None, x2d=None):
        ...
        if x1d_s is not None:
            x_d_s = self.quantizer1d_s.get_codes_from_indices(x1d_s)
            x1d_s = x_d_s.sum(dim=0).permute(0, 2, 1)
            x_out1d_s = self.decoder1d_s(x1d_s)
        # x_out1d_s = rearrange(x_out1d_s, 'b j (t c) -> b j t c', c=6)
        # x_out1d_s = x_out1d_s[:, :, :56, :]
    ...
    def decode_all(self, x1d_s=None, x1d_t=None, x2d=None):
        x_out1d_s = None
        x_out1d_t = None
        if x1d_t is not None:
            x_d_t = self.quantizer1d_t.get_codes_from_indices(x1d_t)
        ...
        x_out1d_s = rearrange(x_out1d_s, ('b j (n t) c -> b n j (t c)'),
                              n=num_clip_t)
        x_out2d_s = self.linear_out_s(
            torch.concat([x_out1d_s, x2d_out_s], dim=3))
